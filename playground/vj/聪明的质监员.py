...
global ans
ans=inf
def check(w,nums,vals):
    idx=0
    for ww,v in stone:
        num=val=0
        if ww>=w: num,val=1,v
        nums[idx+1]=nums[idx]+num
        vals[idx+1]=vals[idx]+val
        idx+=1
    y=0
    for l,r in q:
        y+=(nums[r]-nums[l-1])*(vals[r]-vals[l-1])
    global ans,s
    ans=min(ans,abs(y-s))
    return y>s
...
def solve():
    ...
    for _ in range(m):
        q.append(LII())
    nums=[0 for _ in range(len(stone)+1)]
    vals=[0 for _ in range(len(stone)+1)]
    r+=1
    while l<r:
        mid=l+r>>1
        if check(mid,nums,vals):
            l=mid+1
        else:
            r=mid
    print(ans)
