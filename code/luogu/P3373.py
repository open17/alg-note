...
mark_mul=[1 for _ in range(MAXN<<2)]
a=[0]*MAXN
...
def solve():
    ...
    def push_down(p,size):
        if size<=1: return 
        tree[p<<1]+=mark_mul[p]*ceil(size/2)
        mark_mul[p<<1]+=mark_mul[p]
        tree[p<<1|1]+=mark_mul[p]*(size//2)
        mark_mul[p<<1|1]+=mark_mul[p]
        mark_mul[p]=0
    def push_down_mul(p,size):
        if size<=1: return 
    ...
    def query(l,r,p=1,cl=1,cr=n):
        ...
        mid=(cl+cr)>>1
        ans=0
        if l <= mid: ans+=query(l,r,p<<1,cl,mid)%m
        ans%=m
        if r > mid: ans+=query(l,r,p<<1|1,mid+1,cr)%m
        return ans%m
